...
const SearchResult = ({ query }: any) => {
  ...
  const videos = async () => {
    try {
      ...
      let results = allVideos.filter((vid: any) => {
        const title = (vid.videotitle || "").toLowerCase();
        const channel = (vid.videochanel || "").toLowerCase();
        return queryWords.every((word: string) => title.includes(word) || channel.includes(word));
      });
      setvideos(results);
    } catch (error) {
      console.log(error);
          ...
          {video.map((video: any) => (
            <div key={video._id} className="flex gap-4 group">
              <Link href={`/watch/${video._id}`} className="flex-shrink-0">
                <div className="relative w-80 aspect-video bg-gray-100 dark:bg-zinc-800 rounded-lg overflow-hidden">
                  <video
